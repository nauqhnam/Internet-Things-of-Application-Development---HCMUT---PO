...
<body>
  <div class="card">
    <div class="title">Wi-Fi Settings</div>
    <div class="subtitle">
      Nhập SSID & mật khẩu Wi-Fi STA. ESP32 sẽ thử kết nối rồi cập nhật lại trạng thái.
    </div>
    <form id="wifiForm">
      <label for="ssid">SSID</label>
      <input id="ssid" name="ssid" placeholder="Tên Wi-Fi" required />
    ...
    <div class="footer">
      <span>← <a href="/" style="color:#38bdf8;text-decoration:none;">Back to Dashboard</a></span>
      <span class="teacher">Thầy Sỹ đẹp zai™</span>
    </div>
  </div>
...
document.getElementById('wifiForm').addEventListener('submit', function(e) {
  ...
  statusEl.textContent = 'Đang gửi yêu cầu kết nối...';
  ...
  .then(r => r.text())
  .then(t => {
    statusEl.textContent = 'Server: ' + t + ' · Vui lòng đợi vài giây rồi F5 /status.';
  })
  .catch(err => {
    statusEl.textContent = 'Lỗi: ' + err;
  });
});
// === LOGIC POLLING TRẠNG THÁI (MỚI) ===
function updateStatus() {
  fetch('/status')
    ...
    .then(s => {
      ...
      if (s.wl_status === '3') { // WL_CONNECTED
        statusText = '✅ Đã kết nối!';
      } else if (s.connecting === 'true' || s.wl_status === '1') { // WL_NO_SSID_AVAIL
        statusText = '🔄 Đang tìm kiếm...';
      } else if (s.mode === 'AP') {
        statusText = '💡 AP Mode: ' + ip;
      } else {
        statusText = '❌ Kết nối thất bại (WL Status: ' + s.wl_status + ')';
      }
      
      statusEl.innerHTML = `Trạng thái: <b>${statusText}</b> | IP: <b>${ip}</b>`;

      // Sau khi kết nối thành công, quay về Dashboard
      if (s.wl_status === '3') {
          setTimeout(() => { window.location.href = '/'; }, 3000);
      }
    })
    .catch(err => {
      document.getElementById('status').textContent = 'Lỗi cập nhật trạng thái.';
    });
}

// Bắt đầu kiểm tra trạng thái ngay lập tức và sau mỗi 3 giây
updateStatus();
...
void handleToggle(AsyncWebServerRequest *request)
{
  if (!request->hasArg("led")) {
    request->send(400, "application/json", "{\"error\":\"missing led\"}");
    return;
  }
  ...
  applyLedStates();  // ✅ thực thi

  String res = "{\"led1\":\"" + String(led1_state ? "ON" : "OFF") +
...
void handleConnect(AsyncWebServerRequest *request) {
    ...
    // Đảm bảo bạn đã khai báo extern Save_info_File
    // Save_info_File(request->arg("ssid"), request->arg("password"), CORE_IOT_TOKEN, CORE_IOT_SERVER, CORE_IOT_PORT); 
    
    WIFI_SSID = request->arg("ssid");
    WIFI_PASS = request->arg("password");
    vTaskDelay(pdMS_TO_TICKS(50));
    // Đặt logic khởi động/kết nối vào đây
    Serial.printf("[HTTP] /connect ssid=\"%s\" pass_len=%d\n", WIFI_SSID.c_str(), WIFI_PASS.length());

    request->send(200, "text/plain", "Connecting.");
    isAPMode = false;
    connecting = true;
    connect_start_ms = millis();
    connectToWiFi(); // Bắt đầu quá trình kết nối STA
}
...
void setupServer()
{
  ...
  json += "\"wl_status\":"     + String((int)st) + ",";
  json += "\"ssid\":\""        + WIFI_SSID + "\",";
  json += "\"ip\":\""          + WiFi.localIP().toString() + "\",";
  json += "\"rssi\":"          + String(st == WL_CONNECTED ? WiFi.RSSI() : 0) + ",";
  json += "\"elapsed_ms\":"    + String(connecting ? (millis() - connect_start_ms) : 0);
  json += "}";
...
{
    // CRITICAL FIX 1: Đảm bảo TCP/IP stack đã sẵn sàng trước khi gọi API mạng
    vTaskDelay(pdMS_TO_TICKS(100));
    pinMode(BOOT_PIN, INPUT_PULLUP);
    
    // 2. LOGIC QUYẾT ĐỊNH CHẾ ĐỘ BAN ĐẦU
    if (WIFI_SSID.isEmpty()) {
        Serial.println("[WIFI] Không có cấu hình. Khởi động AP Mode.");
        startAP();
        isAPMode = true;
    } else {
        Serial.println("[WIFI] Có cấu hình. Bắt đầu kết nối STA.");
        connectToWiFi();
        isAPMode = false;
        connecting = true;
        connect_start_ms = millis();
    ...
    // 3. Khởi động Webserver (sau khi Wi-Fi Mode được chọn)
    setupServer();
    ...
    while (1) {
        ...
        if (millis() - hb_ms > 2000) {
            hb_ms = millis();
            wl_status_t st = WiFi.status();
            
            // Fix Vấn đề 3: Chỉ lấy IP khi đã kết nối
            String currentIP = (st == WL_CONNECTED) ? WiFi.localIP().toString() : "0.0.0.0";
            
            Serial.printf("[HB] mode=%s connecting=%d wifi=%d IP=%s\n",
                         isAPMode ? "AP" : "STA",
                         connecting,
                         (int)st,
                         currentIP.c_str());
        ...
        if (digitalRead(BOOT_PIN) == LOW) {
            ...
            if (digitalRead(BOOT_PIN) == LOW && !isAPMode) {
                 Serial.println(">>> Nút BOOT được giữ. Chuyển sang AP.");
                 startAP();
                 isAPMode = true;
                 connecting = false;
                 // Các Task khác sẽ xử lý Delete_info_File()
            }
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // CRITICAL FIX: CẬP NHẬT TRẠNG THÁI NGAY LẬP TỨC
        ...
        Serial.println("[WIFI] Kết nối STA thất bại (Timeout). Chuyển sang AP.");
        startAP();
          }
}
